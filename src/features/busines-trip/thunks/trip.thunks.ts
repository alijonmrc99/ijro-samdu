...
export const fetchTrips = createAsyncThunk('trip/fetchTrips',
    async (params: any, { rejectWithValue }) => {
        try {
            return await httpApi.get<IResponse<IPageable<IBusinessTripResponse>>>(ENDPOINT_BUS_TRIP, params)
        } catch (error) {
            return rejectWithValue(error);
        }
    });
...
export const FetchTripById = createAsyncThunk('trip/FetchById',
    async (id: any, { rejectWithValue }) => {
        try {
            return await httpApi.get<IResponse<IBusinessTripResponse>>(`${ENDPOINT_BUS_TRIP}/${id}`, {})
        } catch (error) {
            return rejectWithValue(error);
        }
    });
